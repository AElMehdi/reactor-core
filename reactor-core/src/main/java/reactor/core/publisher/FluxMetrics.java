...
final class FluxMetrics<T> extends InternalFluxOperator<T, T> {
	...
	static class MetricsSubscriber<T> implements InnerOperator<T, T> {
		...
		final public void onComplete() {
			...
			// because it would skew the onNext count by one.
			// We differentiate between empty completion and value completion
	 ...
	 * A status tag is added to specify what event caused the timer to end (onComplete, onCompleteEmpty, onError, cancel).
	 */
